...
export async function createAssignment({
  ...
  const isOverride = dueDriverId && plannedDriverId && dueDriverId !== plannedDriverId;
...
export async function confirm(assignmentId, reason = '') {
  ...
  const override = a.plannedDriverId !== a.dueDriverId;
  if (override && !reason.trim()) throw new Error('سبب التجاوز مطلوب');

  const next = {
    ...a,
    actualDriverId: a.plannedDriverId,
    status: override ? ASG_STATUS.OVERRIDDEN : ASG_STATUS.CONFIRMED,
    replacementReason: override ? reason : (a.replacementReason || null),
